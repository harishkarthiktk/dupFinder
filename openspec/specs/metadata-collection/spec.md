...
The system SHALL extract essential metadata from each discovered file, including filename, absolute path, file size, modification time (modified_time), and scan date.

#### Scenario: Successful Metadata Collection
- **WHEN** a file is discovered during scanning
- **THEN** the filename, absolute path, size in bytes, modified_time as Unix timestamp, and current scan date are collected and associated with the file

#### Scenario: Error in Metadata Retrieval
- **WHEN** an error occurs while retrieving metadata (e.g., permission denied for size or modified_time)
- **THEN** an error is logged, default values are used (e.g., size=-1, modified_time=0.0), and the file is still included in the scan with available metadata
...
- **THEN** absolute paths are correctly formed using os.path, and modified_time is retrieved as float Unix timestamp
