...
- In `utilities/database.py`, modify `initialize_database`:
...
- Ensure backward compatibility: Add/alter column if needed (though type change requires migration).

## Task 2: Implement Migration Function
- Add `migrate_scan_date_to_epoch()` in `utilities/database.py`:
  - Connect to DB.
...
- Call this in `initialize_database` if migration needed.
- Add unit test for migration on sample data.

## Task 3: Update Storage Functions
- In `utilities/database.py`:
...
- Import `import time` where needed.
...
- In `utilities/html_generator.py`:
...
- Use `datetime.fromtimestamp(epoch).astimezone()` for explicit local timezone (Asia/Calcutta or system default).
- For reports: Format as '%Y-%m-%d %H:%M:%S %Z' to include timezone.
- Test with different system timezones if possible.
...
- Run full integration test: Scan, generate report, verify dates.

## Task 8: Documentation and Cleanup
- Update `README.md` if CLI options affected (none).
- Update `openspec/specs/database-management/spec.md` with new schema details.
- Deprecate any string-based time handling.
